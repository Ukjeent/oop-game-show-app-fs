class Game {
  ...
  handleInteraction(key) {
    if (!phrase.checkLetter(key)) {
      // Checks if any letters in the phrase matches the clicked letter before adding the correct class and displaying the letter on the site.
  ...
  checkForWin() {
    ...
    if (lettersArray.every((letter) => letter.classList.contains("show"))) {
      // Returns true if all letters contains the 'show' class
  ...
  removeLife() {
    ...
    if (this.missed === 4) {
      // Calls the gameover function if the player misses more 5 times.
